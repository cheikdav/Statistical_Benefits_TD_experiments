...
class MC_TD_summary():
    def __init__(self, timestamps, M):
        self.MC_estimates = defaultdict(float)
        self.P_estimates = defaultdict(self.empty_dic)
        self.R_estimates = defaultdict(float)
        self.N_visits = defaultdict(int)
        self.N_observations = 0
    ...
    def add_trajectory(self, t):
        ...
        for (s,r,sb) in t:
            ...
            if sb not in self.index:
                self.index[sb] = len(self.index)
            tot_rew += r
            self.N_visits[s] +=1
            self.P_estimates[s][sb] += 1
            self.R_estimates[s] += r
        self.MC_estimates[t[0][0]] += tot_rew
        self.N_observations += 1
        if self.N_observations in self.timestamps:
            self.archive_TD_estimates.append(self.TD_estimation())
            self.archive_MC_estimates.append(self.MC_estimation())

    def TD_estimation(self):
        S = len(self.index)
        P = np.zeros((S, S))
        R = np.zeros(S)
        for s, succ in self.P_estimates.items():
            for s_, num_trans in succ.items():
                P[self.index[s]][self.index[s_]] = num_trans/self.N_visits[s]
            R[self.index[s]] = self.R_estimates[s]/self.N_visits[s]
        V = np.linalg.inv(np.eye(S) - P ) @ R
        return defaultdict(float,{s:V[k] for s,k in self.index.items() if s in self.M.I})

    def MC_estimation(self):
        return defaultdict(float,{s: self.MC_estimates[s]/self.N_visits[s] for s in self.MC_estimates})
